...
object Base58 {
    ...
    fun encodeChecked(version: Int, payload: ByteArray): String {
        ...
        val checksum: ByteArray = Sha256Hash.hashTwice(addressBytes, 0, payload.size + 1)
    ...
    fun decode(input: String): ByteArray {
        ...
        for (i in 0 until input.length) {
            ...
            require(digit >= 0) { "Invalid character '" + Character.toString(c) + "' at position " + i }
    ...
    fun decodeChecked(input: String): ByteArray {
        ...
        require(decoded.size >= 4) { "Input too short: " + decoded.size }
        val data = Arrays.copyOfRange(decoded, 0, decoded.size - 4)
        val checksum = Arrays.copyOfRange(decoded, decoded.size - 4, decoded.size)
        val actualChecksum: ByteArray = Arrays.copyOfRange(Sha256Hash.hashTwice(data), 0, 4)
        require(Arrays.equals(checksum, actualChecksum)) { "Invalid checksum" }
        return data
    }
